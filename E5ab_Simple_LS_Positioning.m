...
%readRinex_obs_2024_GAL(obsfilename); 
...
obs_data = [obsfilename(1:end-4), '_GAL.mat'];
load(obs_data);
[gal_ephem, ionoGAL] = read_clean_GALbroadcast(navfilename,1);
...
hr = 9;
min = 35; %7h0m %7h10m
tow_start = min*60 + hr*3600 + 86400*2; 
hr = 9;
min = 35; %7h10m %8h5m
...
while iter < 6
    ...
    %elevation angles
    %azimuth angles
    [R_exp, rGAL, bsv, relsv, all_el, all_az] = expectedRange_GAL(c, guess_RX_loc, wn, tow, t_input, gal_ephem);
    epoch = tow;
    ionoklobuchar = klobuchar(ionoGAL, epoch, lat, lon, all_az, all_el);
    ionoklobuchar = ionoklobuchar.*(1575.42/1207.14)^2;
    ...
    n = 1;
    for i = 2:36 %PRN 1 is out of commission right now
        if all_el(i) > 10
           index(n) = i;
           n = n + 1;
        end
    ...
    for i = 1:channels
        A(i,1) = -(RGAL(i,1) - guess_RX_loc(1))/R(i);
        A(i,2) = -(RGAL(i,2) - guess_RX_loc(2))/R(i);
        A(i,3) = -(RGAL(i,3) - guess_RX_loc(3))/R(i);
        A(i,4) = 1;
    end
...
% ylabel("Residual [m]");
% title("E5a RX5 Position Residuals vs. Time No RFI");
% grid on
% hold off
% 
% 
...
% % xlabel("Time since 07:00 UTC 9/10/24 [min]");
% % ylabel("Residual [m]");
% % title("E5a RX3 Position Residuals vs. Time (Jamming + Meaconing)");
% % grid on
% % hold off
% % 
% % % splot clock bias
