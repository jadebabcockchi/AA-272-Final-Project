function [health,x,bsv,relsv] = broadcast_eph2pos_modified(ephem_all,t_input,prn)
...
bsv        = ones(sz,1) * NaN; %clock bias correction
relsv      = ones(sz,1) * NaN; %relativity correction
...
%     jj = max( find(dt_input(tt) >= dt_ephem) ); % sat_ephem(:,17) = toe (sec into GPS week) of each entry
    ...
    bsv(tt,1) = c*(sat_ephem(jj,21) + sat_ephem(jj,22).*dt + sat_ephem(jj,23).*dt.^2);
    relsv(tt,1) = -4.4428e-10*ecc*sqrt(a)*sin(E)*c;
    ...
    uDelt = sat_ephem(jj,12)*sin(2*u) + sat_ephem(jj,11)*cos(2*u);
    rDelt = sat_ephem(jj,14)*sin(2*u) + sat_ephem(jj,13)*cos(2*u);
    iDelt = sat_ephem(jj,16)*sin(2*u) + sat_ephem(jj,15)*cos(2*u);
    
    r = a * (1 - ecc*cosE);     % radius  
    i = sat_ephem(jj,7) ;       %  inclination 
    i_dot = sat_ephem(jj,10);   %  inclination rate
    
    uCorr = u + uDelt;
    rCorr = r + rDelt;
    iCorr = i + iDelt + i_dot*dt;                                            

    % Compute satellite position in orbital plane (Eq. 13)
    %======================================================================
    
    xo = rCorr * cos(uCorr);    % satellite x-position in orbital plane
    yo = rCorr * sin(uCorr);    % satellite y-position in orbital plane
    ...
    x(tt,1) = xo*cos(node) - yo*cos(iCorr)*sin(node);  %x-position  

    x(tt,2) = xo*sin(node) + yo*cos(iCorr)*cos(node);  %y-position 

    x(tt,3) = yo*sin(iCorr);                           %z-position
