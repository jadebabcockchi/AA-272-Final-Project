function [R_exp, rGPS, bsv, relsv, all_el, all_az] = expectedRange(c, rRX, wn, tow, t_input, gps_ephem)
    %step 1: compute gps satellite postion in ECEF based on broadcast ephem.
    
    guess = rRX;
    for i = 1:32
        [~,pos,bsv(i),relsv(i)] = broadcast_eph2pos_modified(gps_ephem,t_input,i);
    ...
    while (norm(noNaN) > 1e-5)
        ...
        t_input = [wn Tt];
        for i = 1:32
            [~,pos,bsv(i),relsv(i)] = broadcast_eph2pos_modified(gps_ephem,t_input(i,:),i);
            rGPS_adj1(i,1) = pos(1);
            rGPS_adj1(i,2) = pos(2);
            rGPS_adj1(i,3) = pos(3);
        end
